...
export default function Profile() {
  ...
  const myOrders = useQuery(api.orders.getMyOrders, user ? { clerkId: user.id } : "skip") || [];
    ...
    <div className="w-full bg-gray-50 min-h-screen pb-6">
      ...
      <div className="bg-white px-4 pt-10 pb-6 rounded-b-[30px] shadow-sm relative">
        ...
        <div className="flex items-center gap-4 mb-6">
          <img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${user?.primaryEmailAddress?.emailAddress}`} alt={displayName} className="w-20 h-20 rounded-full border-4 border-brand-50 shadow-sm bg-gray-100" />
          <div>
